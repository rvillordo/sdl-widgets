...
{
	//image_t *image = image_load_png("./teste.png");
	/*image_t *scale = image_alloc(image->w/2, image->h/2, 1, 0);
	image_scale_down(image, scale);
	printf("IMAGE: %dx%d\n", image->w, image->h);
	printf("IMAGE: %dx%d\n", scale->w, scale->h);*/
	int x, y, i;
	int r, g, b;
	int yy, xx, kk=0;
	image_t *stick[128];
	image_t *screen = image_alloc(189, 48, 8, 0);
	image_t *sprite = image_load_ppm("sticky.pnm");
	memset(screen->data, ' ', screen->scanline * screen->h);
	for(y = 0; y < screen->h; y++) {
		for(x = 0; x < screen->w; x+=2) {
			screen->data[(y * screen->scanline) + x] = 255;
			screen->data[(y * screen->scanline) + x + 1] = 255;
		}
	}

	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 0, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 32, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64+32, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64+64, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64+64+32, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64+64+64, 32, 32);
	for(i = 0; i < 8; i++)
		stick[kk++] = image_crop(sprite, 32*i, 64+64+64+32, 32, 32);
	
	
	
	/*stick[0] = image_load_ppm("s0.pnm");
	stick[1] = image_load_ppm("s1.pnm");
	stick[2] = image_load_ppm("s2.pnm");
	stick[3] = image_load_ppm("s3.pnm");
	stick[4] = image_load_ppm("s4.pnm");*/


	compute_xterm256_cmap(256);
	image_t *image;
	pixel_t pixel;
	printf("\033[?25l");
	int px = screen->w/3;
	...
	int spr = 56-40;
	for(i=spr;;i++) {
		memset(screen->data, 255, screen->scanline * screen->h);
		if(!(tmm++%2))
			i++;
		if((i > (spr + 7))) { //(kk-1))) {
			//usleep(500000);
		...
		image = stick[i];
		for(yy=1, y = 5; y < image->h; y++, yy++) {
				for(xx=(1*1), x = 0; x < image->scanline; x+=(image->bpp/8), xx++) {
					//pixel = image_get_pixel(image, x, y);
					pixel.color = image->data[(y * image->scanline) + x];
					//if(pixel.color==255)
					//	continue;
					//r = image->data[(y * image->scanline) + x];// image->palette[image->data[(y*(image->scanline))+x]];
					//g = image->data[(y * image->scanline) + x + 1];//= image->palette[image->data[(y*(image->scanline))+x+1]];
					//b = image->data[(y*(image->scanline))+x+2];
				//	printf("\033[%d;%dH", yy+y, xx+x);
					putpix(screen, xx,yy,pixel);

					//if(screen->data[((yy + y) * screen->w) + xx+x] == 255)
					//	screen->data[((yy + y) * screen->w) + xx+x]=0;
				//	printf("\033[38;5;%dm\xdb\033[0m", pixel.color); //find_color(r, g, b));
				}
			}
			//usleep(atoi(argv[1]));
		//}
		for(y = 0; y < screen->h; y++) {
			for(x = 0; x < screen->w; x++) {
				if(y>26&& y < 28) {
					...
					screen->data[(y*screen->scanline)+px+1]=0;
					screen->data[((y+1)*screen->scanline)+px]=0;
					screen->data[(y*screen->scanline)+px-1]=0;
				}
				printf("\033[%d;%dH", y, x);
				printf("\033[38;5;%dm.", screen->data[(y * screen->scanline) + x]?0:255);
				fflush(stdout);
			}
		}
	}
